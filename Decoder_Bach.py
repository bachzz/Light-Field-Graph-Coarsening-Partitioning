...
def decoder_run(filename_coder_output, coeff_reduced_filename):      
    ...
    Coeff_reduced_decoded = f[T_coeffkey] #f1['Coeff_reduced_decode']
    ...
    Coeff_reduced_decoded = [Coeff_reduced_decoded[0][i] for i in Idx_reduced] ## bach
    Coeff_reduced_decoded_SR_Q = Coeff_reduced_decoded[:] ## bach
...
#    ray.init()
    ...
    for i_SR in range(len(Con_SR_reduced)):
        ...
        Coeff_decoded_SR = [r[0] for r in Coeff_reduced_decoded_SR_Q]
        Coeff_decoded_SR_ = [Coeff_decoded_SR[qidx][~np.isnan(Coeff_decoded_SR[qidx])] for qidx in range(len(Coeff_decoded_SR))]
        result_ids.append(SR_decode.remote(i_SR,Coeff_decoded_SR_[i_SR], Con_SR_reduced[i_SR], Cmtx_reduced[i_SR], Lum_SR_reduced[i_SR]))
...
@ray.remote
def SR_decode(i_SR,T_coeff_decoded, W_SR, Cmtx_SR, Lum_SR):
    print(i_SR)
...
#filename = 'Results/Results_coarsening_algo_Ncutnmax_5000psnr_min_20_1000_total_variation_dataset_Friendsreduction1Ncut1_coder_output.mat'
filename = 'Results/Results_coarsening_algo_Ncutnmax_5000psnr_min_20_1000_total_variation_dataset_Fountain_Vincent2reduction1Ncut1_coder_output.mat'
