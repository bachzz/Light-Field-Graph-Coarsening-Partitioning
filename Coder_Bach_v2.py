...
#   Start 4 tasks in parallel.
    ...
    img_dict = {
        '0_0': np.round(np.squeeze(Luminance_LF[0,0,:,:])),
        '0_4': np.round(np.squeeze(Luminance_LF[0,4,:,:])),
        '4_0': np.round(np.squeeze(Luminance_LF[4,0,:,:])),
        '4_4': np.round(np.squeeze(Luminance_LF[4,4,:,:]))
    }
    
    depth_dict = {
        '0_0': np.squeeze(Depthset[0,0,:,:]),
        '0_4': np.squeeze(Depthset[0,4,:,:]),
        '4_0': np.squeeze(Depthset[4,0,:,:]),
        '4_4': np.squeeze(Depthset[4,4,:,:])
    }

    img_dict_id = ray.put(img_dict)
    depth_dict_id = ray.put(depth_dict)
...
#        
    ...
    for i_SR in range(1,len(ConMatrixCell[0])+1): ## bach's fix
        if flag == 1:
            result_ids.append(SR_process.remote(i_SR, depth_dict_id, img_dict_id, label_id, np.round(255*ColorMatrixCell[0][i_SR-1]).astype(np.uint8), ConMatrixCell[0][i_SR-1], model_TV, std_scale, n_max, PSNR_min, cut_algo, lambda_, method_rate_coef,enable_reduction,enable_Ncut, weight_flag, method_coarsening, k, psnr_computation))
        else:
            result_ids.append(SR_process.remote(i_SR, depth_dict_id, img_dict_id, label_id, np.round(ColorMatrixCell[0][i_SR-1]).astype(np.uint8), ConMatrixCell[0][i_SR-1], model_TV, std_scale,n_max, PSNR_min, cut_algo, lambda_, method_rate_coef,enable_reduction,enable_Ncut, weight_flag, method_coarsening, k, psnr_computation))
...
@ray.remote
def SR_process(i_SR,Depth_dict,img_dict,Label_map,Color,A,model_TV,std_scale,n_max=15000,PSNR_min=30,cut_algo = 'Ncut',lambda_=300,method_rate_coef='total_variation',enable_reduction=1,enable_Ncut=1,weight_flag=0,method_coarsening='variation_edges',k=10,psnr_computation='lifting'):
    print(i_SR)
    n = Color.shape[1]
    ...
    if ni <= 10:
        ...
        # Essayer le coarsening avec r
            ...
            if distortion > PSNR_min:
                ...
                mask = Label_map==i_SR
                [T_Coeff_SR, Con_SR, Seg_SR, Disp_SR, ID_hiearchy, delta_Rate_SR, Coarsening_all_stages_SR,Cmtx_SR,Lum_SR] = perform_normalized_cut(img_dict, Label_map, Depth_dict, x, G, model_TV, std_scale, n_max, i_SR, cut_algo, PSNR_min,lambda_,method_rate_coef,enable_reduction,enable_Ncut)
        else:
            mask = Label_map==i_SR
            [T_Coeff_SR, Con_SR, Seg_SR, Disp_SR, ID_hiearchy ,delta_Rate_SR, Coarsening_all_stages_SR,Cmtx_SR,Lum_SR] = perform_normalized_cut(img_dict, Label_map, Depth_dict, x, G, model_TV, std_scale, n_max, i_SR, cut_algo, PSNR_min,lambda_,method_rate_coef,enable_reduction,enable_Ncut)
...
def perform_normalized_cut(img_dict, Label_map, disp_dict, Luminance_SR, G, model_TV, std_scale, bArea, label_SR, cut_algo, PSNR_min,lambda_,method_rate_coef,enable_reduction=1,enable_Ncut=1,k=10,M=9,N=9):
    ...
    [T_Coeff_SR, Con_SR, Seg, Disp, ID, Rate, Coarsening_all_stages,Cmtx,Lum_SR] = Ncut_Partition(img_dict, mask, disp_dict, G, Luminance_SR, model_TV, std_scale, bArea, list_, M, N,cut_algo, PSNR_min,lambda_,method_rate_coef,enable_reduction,enable_Ncut)
...
#%% Function to perform recursive Ncut on a specific super-ray              
def Ncut_Partition(img_dict, mask, disp_dict, G, Luminance_SR, model_TV, std_scale, bArea, ID, M, N, cut_algo, PSNR_min,lambda_,method_rate_coef,enable_reduction=1,enable_Ncut=1, k=10,method_coarsening='variation_edges',psnr_computation='lifting'):
        ...
        if ni > bArea and ni<100000:
           ...
           # print(distortion)
            if distortion > PSNR_min:
                ...
                I = np.ravel_multi_index(mask.nonzero(),mask.shape)
                temp_depth = disp_dict['0_0']*np.squeeze(mask[0,0,:,:]) ### NOT SURE
                temp_depth = np.median(temp_depth)   
                Seg = []
                Lum_SR = []
                Cmtx = []
    ...
    if (np.sum(np.squeeze(mask[0,0,:,:])) < 10):    
        ...
        label_im_or_dict = {}
        label_im_dict = {}
        Boundary_rate_max = -9999

        for view_id in img_dict:
            i_ref = int(view_id.split('_')[0])
            j_ref = int(view_id.split('_')[1])

            label_im_or_dict[view_id] = np.full([mask.shape[2], mask.shape[3]], -1.)
            label_im_or_dict[view_id][np.squeeze(mask[i_ref,j_ref,:,:])] = 0
            
            if (cut_algo =='Ncut'):
                graph = image.img_to_graph(img_dict[view_id], mask=np.squeeze(mask[i_ref,j_ref,:,:]))
                graph2 = image.img_to_graph(disp_dict[view_id], mask=np.squeeze(mask[i_ref,j_ref,:,:]))
                ...
                label_im_dict[view_id] = np.full([mask.shape[2],mask.shape[3]], -1.)
                label_im_dict[view_id][np.squeeze(mask[i_ref,j_ref,:,:])] = labels.astype('uint32') 
            ...
            Boundary_rate_max = max(Boundary_rate_max, Boundary_rate)
        ###

        #### visualize code below purpose before continue to edit !!
        ...
        [LabelSquence, disparity_values, ConMatrixCell, PxlIndCell] = Projection_labels(label_im_dict, mask, disp_dict) ### project with dict
        ...
        R_B = Boundary_rate_max ### NOT SURE
...
def Projection_labels_v0(Segmentation_before, mask, disp, compute_color = 0, LF_RGB = [np.nan]):
    # segmentation before 4 D
    # mask 4D 
    # disp 2D
    # Segmentation projection from top-left corner view to the whole LF
...
#filename = 'Results/GBT_LF_Friends_0QP_81views_500numSR_original.mat'
#filename = 'Results/GBT_LF_Fountain_Vincent2_0QP_81views_1000numSR_original.mat'
filename = 'Results/GBT_LF_Greek_0QP_81views_1000numSR_original.mat'
#filename = '/home/mrizkall/Desktop/Project_Graph_Code/Datasets_graphs/GBT_LF_'+dataset+'_0QP_64views_500numSR_original.mat'
filename_model = 'Model/model_SVM.sav'
filename_scaler = 'Model/scaler.sav'
