...
#        
    ...
    for i_SR in range(1,len(ConMatrixCell[0])+1): ## bach's fix
        if flag == 1:
            result_ids.append(SR_process.remote(i_SR, depth_id, img_id, label_id, np.round(255*ColorMatrixCell[0][i_SR-1]).astype(np.uint8), ConMatrixCell[0][i_SR-1], model_TV, std_scale, n_max, PSNR_min, cut_algo, lambda_, method_rate_coef,enable_reduction,enable_Ncut, weight_flag, method_coarsening, k, psnr_computation))
        else:
            result_ids.append(SR_process.remote(i_SR, depth_id, img_id, label_id, np.round(ColorMatrixCell[0][i_SR-1]).astype(np.uint8), ConMatrixCell[0][i_SR-1], model_TV, std_scale,n_max, PSNR_min, cut_algo, lambda_, method_rate_coef,enable_reduction,enable_Ncut, weight_flag, method_coarsening, k, psnr_computation))
